...
export default function ImeiPdfManager() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire("Success!", "IMEI & Document Uploaded!", "success");
      setIsModalOpen(false);
      setFormData(initialForm);
      fetchRecords();
    } catch (err) {
      Swal.fire("Error", "Failed! This IMEI might already exist.", "error");
    } finally {
      setLoading(false);
    }
  };
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-1.5 text-center pr-6 whitespace-nowrap">
                        {r.invoice_pdf ? (
                          <a
                            href={r.invoice_pdf}
                            target="_blank"
