...
export default function TrackId() {
  ...
  return (
    <div className="bg-transparent min-h-screen font-sans pb-10">
      ...
      {/* PERMANENT RESULTS TABLE (High-Density & Flexible Text Wrap) */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative">
        <div className="overflow-x-auto min-h-[60vh] max-h-[calc(100vh-100px)] custom-table-scrollbar">
          {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50/80 border-b border-gray-200 text-slate-500 text-[11px] font-bold uppercase tracking-widest sticky top-0 z-10 backdrop-blur-md whitespace-nowrap">
                <tr>
                  <th className="px-3 py-2 w-12 text-center">S.No</th>
                  <th className="px-3 py-2 text-center bg-indigo-50/50">
                    Tracking ID
                  </th>
                  <th className="px-3 py-2">Order ID</th>
                  <th className="px-3 py-2">Invoice No</th>
                  <th className="px-3 py-2">Invoice Date</th>
                  <th className="px-3 py-2 min-w-[200px]">Model Name</th>
                  <th className="px-3 py-2 text-center">Inv Qty</th>
                  <th className="px-3 py-2 text-right">Inv Amount</th>
                  <th className="px-3 py-2 border-l border-gray-100">Firm</th>
                  <th className="px-3 py-2">Location</th>
                  <th className="px-3 py-2 text-center border-l border-gray-100">
                    Status
                  </th>
                </tr>
              </thead>
                  ...
                  results.map((item, index) => {
                    ...
                    return (
                      <tr
                        ...
                        className="hover:bg-slate-50/50 transition-colors group"
                      >
                        <td className="px-3 py-1.5 text-center font-mono text-[13px] font-bold text-slate-400 whitespace-nowrap">
                          {(index + 1).toString().padStart(2, "0")}
                        </td>
                        <td className="px-3 py-1.5 font-black text-indigo-600 bg-indigo-50/20 text-[12px] uppercase tracking-wider text-center whitespace-nowrap">
                          {item.tracking_id || trackingId}
                        </td>
                        <td className="px-3 py-1.5 font-bold text-slate-800 text-[13px] whitespace-nowrap">
                          {item.order_id || "-"}
                        </td>
                        <td className="px-3 py-1.5 font-bold text-slate-700 text-[13px] whitespace-nowrap">
                          {item.invoice_no || "-"}
                        </td>
                        <td className="px-3 py-1.5 text-slate-600 font-medium text-[13px] whitespace-nowrap">
                          {formatDate(item.invoice_date)}
                        </td>

                        {/* 🔥 TEXT WRAP APPLIED HERE 🔥 */}
                        <td className="px-3 py-1.5 font-bold text-slate-700 text-[13px] whitespace-normal min-w-[200px] max-w-[300px] break-words leading-snug">
                          {item.model_name || "-"}
                        </td>
                        ...
                        <td className="px-3 py-1.5 text-right font-bold text-slate-800 text-[13px] whitespace-nowrap">
                          ₹
                          {parseFloat(item.invoice_amount || 0).toLocaleString(
                            "en-IN",
                          )}
                        ...
                        {/* 🔥 TEXT WRAP APPLIED HERE 🔥 */}
                        <td className="px-3 py-1.5 font-bold text-slate-700 text-[13px] border-l border-gray-100 whitespace-normal min-w-[120px] max-w-[200px] break-words leading-tight">
                          {item.firm || "-"}
                        </td>
                        <td className="px-3 py-1.5 text-slate-600 text-[13px] whitespace-normal min-w-[100px] max-w-[150px] break-words leading-tight">
                          {item.location || "-"}
                        </td>

                        {/* Status Badge */}
                        <td className="px-3 py-1.5 text-center border-l border-gray-100 whitespace-nowrap">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-bold tracking-widest uppercase border border-dashed ${badgeStyle.bg}`}
                          >
                            <span
